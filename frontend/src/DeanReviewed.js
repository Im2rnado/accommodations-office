...
const DeanReviewed = () => {
    ...
    useEffect(() => {
        ...
        // Fetch students from the dean's faculty that have been reviewed
        axios
            .get(`http://localhost:4000/api/students`)
            .then((response) => {
                if (response.data.success) {
                    // Filter students by faculty and those that have been reviewed by this dean
                    const reviewedStudents = response.data.students.filter(student =>
                        student.faculty === faculty &&
                        student.deanReviewedBy === deanData._id
                    );

                    setStudents(reviewedStudents);
                    setFilteredStudents(reviewedStudents);
                } else {
        ...
        <div className="flex min-h-screen bg-gray-900 text-white">
            ...
            <aside className="w-64 bg-[#072D4A]/90 flex flex-col">
                ...
                <nav className="m-2 space-y-4">
                    ...
                    <NavLink to="/dean-reviewed" className="block py-2 px-4 bg-[#007ECA]/70 text-white hover:bg-[#007ECA]/60 rounded-xl"> Reviewed </NavLink>
                </nav>
            ...
            <main className="flex-1 bg-blue-100 p-8">
                <h1 className="text-2xl font-bold mb-6 text-black">Reviewed Students - {faculty} Faculty</h1>
                ...
                <div className="grid grid-cols-3 gap-6">
                    {filteredStudents.length > 0 ? (
                            ...
                            <NavLink key={student.id} to={`/dean-review/${student.id}`}>
                                <div className="bg-[#007ECA] rounded-xl p-3 flex items-start space-x-3 shadow-lg">
                                    ...
                                    <div className="flex-grow min-w-0">
                                        ...
                                        <span className={`inline-block mt-1 ${student.status === 'Approved' ? 'bg-green-500' : 'bg-red-500'} text-white text-xs px-2 py-1 rounded-xl`}>
                                            {student.status}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div className="col-span-3 text-center p-6 bg-white rounded-xl shadow-lg">
                            <p className="text-black text-xl">No reviewed students in {faculty} faculty.</p>
                        </div>
                    )}
                </div>
            </main>
