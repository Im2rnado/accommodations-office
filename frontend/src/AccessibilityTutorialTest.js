...
function AccessabilityTutorial()
{
    return (
        <div className="flex flex-row w-full min-h-screen bg-gray-900 text-white">
            ...
            <div id="moduleBox" className="w-[80%] h-screen flex flex-col justify-start items-center overflow-y-scroll">
                ...
                <div className="w-[60%] h-fit bg-white z-10"></div>

                <button id="nextButton" onClick={()=>changeModule()} className="w-48 pt-2 pb-2 m-auto mb-6 bg-blue-500 text-white border-solid border-4 transition-all duration-200
...
{   
    moduleOrder++;
    let key = moduleOrder;
    let keyName = keyArray[key];
    let moduleContent = moduleArray[keyName];
    let moduleTitle = document.getElementById("moduleTitle");
    let nextButton = document.getElementById("nextButton");

    moduleTitle.innerHTML = `<i class="fa-solid fa-list mr-3"></i>` + keyName;

    document.getElementById("moduleBox").innerHTML = moduleTitle.outerHTML + moduleContent + nextButton.outerHTML;
...
let moduleOrder = 1;
let keyArray = {
    1:"Welcome to ESLSCA University",
    2:"Accessibility Button"
}
let moduleArray = {
    "Welcome to ESLSCA University":`<p className="text-2xl w-[90%] h-fit mt-6 mb-6">Congratulations! Now you have become a part of ESLSCA's family</p>
    ...
    "Accessibility Button":`<p className="text-2xl w-[90%] h-fit mt-6 mb-6">
